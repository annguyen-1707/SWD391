  ...
  if (userRole !== 'PlatformAdmin') {
    return (
      <div className="flex flex-col items-center justify-center p-12 mt-12 text-center bg-white rounded-2xl shadow-sm border border-slate-200">
        ...
        <p className="text-slate-600 mb-6">You do not have permission to access the Platform Administrator Dashboard. Your current role is {userRole || 'Guest'}.</p>
  ...
  return (
    <div className="max-w-2xl mx-auto py-8">
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
        ...
        <div className="p-8">
          {status.message && (
            <div className={`mb-6 p-4 rounded-xl flex items-start space-x-3 ${status.type === 'success' ? 'bg-emerald-50 text-emerald-800 border border-emerald-200' : 'bg-red-50 text-red-800 border border-red-200'}`}>
              {status.type === 'success' ? <CheckCircle className="w-5 h-5 mt-0.5 text-emerald-500" /> : <XCircle className="w-5 h-5 mt-0.5 text-red-500" />}
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-2">
              ...
              >
                <option value={1}>Platform Admin</option>
